    ...
    if (this.pageConfig.showResults) {
      ...
      for (i = 0; i < trials.length; ++i) {
        ...
        if (trial.type === "headphones_screen"){
          ...
          thT = $("<th colspan='3'></th>");
          $(thT).append(trial.id + " (Headphones Screening)" );
          $(trT).append(thT);
          $(table).append(trT);
          trLabel = document.createElement("tr"); 
          thT = $("<th>" + this.pageManager.getLocalizer().getFragment(this.language, 'response') + "</th>");
          $(trLabel).append(thT);
          $(table).append(trLabel);

          var j;
          for(j = 0; j < trial.responses.length; ++j){
            trHS = document.createElement("tr"); 
            tdHSCorrect = document.createElement("td"); 
            var response = trial.responses[j];         
            
            $(tdHSCorrect).append(response.correct);
            $(trHS).append(tdHSCorrect);
            $(table).append(trHS);
          }
          trEmpty = $("<tr height='8px'></tr>");
          $(table).append(trEmpty);
        } else if (trial.type === "mushra") {
          trT = document.createElement("tr");        
          thT = $("<th colspan='2'></th>");
