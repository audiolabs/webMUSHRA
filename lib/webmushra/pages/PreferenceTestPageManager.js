...
PreferenceTestPageManager.prototype.createPages = function (_pageManager, _pageTemplateRenderer, _pageConfig, _audioContext, _bufferSize, _audioFileLoader, _session, _errorHandler, _language) {
  ...
  this.stimuli = [];
  this.trials = [];
  /* If the stimuli are passed in groups, only create pairs from the items within each group */
  if (Array.isArray(_pageConfig.stimuli)){
      for (group of _pageConfig.stimuli){
          this.stimuli = [];
          for (var key in group) {
              this.stimuli[this.stimuli.length] = new Stimulus(key, group[key]);
          }
          for (pair of pairs(this.stimuli, _pageConfig.considerOrder)){
            this.trials.push(pair);
          }
      }
  } else { /* Otherwise make pairs of all stimuli */
